...
var INPUT_ERR, SEND_TX_ERR, STAKE_BOTH_SIDES_ERR, Web3, againstProposalList, allowEmergencyWithdraw, avgROI, betoken, betoken_addr, chart, checkIfStakedOnOtherSide, checkKairoAmountError, clock, commissionRate, copyTextToClipboard, countdownDay, countdownHour, countdownMin, countdownSec, cycleNumber, cyclePhase, cycleTotalForStake, displayedKairoBalance, displayedKairoUnit, errorMessage, hasWeb3, kairoAddr, kairoBalance, kairoTotalSupply, kyberAddr, loadFundData, memberList, minStakeProportion, networkName, networkPrefix, oraclizeAddr, paused, prevCommission, prevROI, proposalList, showCountdown, showError, showSuccess, showTransaction, startTimeOfCyclePhase, successMessage, supportedProposalList, timeBetweenCycles, timeOfChangeMaking, timeOfFinalizing, timeOfWaiting, totalCommission, totalFunds, transactionHash, transactionHistory, userAddress, userBalance, web3;
...
Template.proposals_tab.helpers({
  ...
  is_disabled: function(_id, _isSold, _buyPrice) {
    if (_id === "support" || _id === "against") {
      ...
      if ((cyclePhase.get() !== 1 && cyclePhase.get() !== 2) || (cyclePhase.get() === 1 && _buyPrice > 0) || (cyclePhase.get() === 2 && _isSold === true)) {
        return "disabled";
      }
    }
    return "";
...
Template.proposals_tab.events({
  ...
  "click .execute_proposal": function(event) {
    ...
    switch (cyclePhase.get()) {
      case 1:
        return betoken.executeProposal(id, showTransaction);
      case 2:
