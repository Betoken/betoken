...
var ROIList, ROIRawData, Web3, betoken, betoken_addr, chart, clock, commissionRate, countdownDay, countdownHour, countdownMin, countdownSec, cyclePhase, devFeeProportion, displayedKairoBalance, displayedKairoUnit, etherDelta_addr, getCurrentAccount, kairoBalance, kairoTotalSupply, kairo_addr, loadFundData, memberList, networkName, proposalList, showCountdown, showTransaction, startTimeOfCycle, supportedProposalList, timeOfChangeMaking, timeOfCycle, timeOfProposalMaking, totalFunds, transactionHash, userAddress, userBalance, web3;
...
loadFundData = function() {
  ...
  getCurrentAccount().then(function(_userAddress) {
    ...
    betoken.contracts.groupFund.events.ROI({
      ...
      chart.data.datasets[0].data.sort(function(a, b) {
        return Number.parseInt(a.x) - Number.parseInt(b.x);
      });
      chart.update();
      rawData = ROIRawData.get();
      rawData.push(data);
      rawData.sort(function(a, b) {
        return Number.parseInt(a._cycleNumber) - Number.parseInt(b._cycleNumber);
      });
      ROIRawData.set(rawData);
      return ROIList.set(chart.data.datasets[0].data);
    });
    //Example data
    chart.data.datasets[0].data = [
      {
  ...
  betoken.getPrimitiveVar('developerFeeProportion').then(function(_result) {
    return devFeeProportion.set(+_result);
  });
  betoken.getPrimitiveVar('commissionRate').then(function(_result) {
    return commissionRate.set(+_result);
  });
  //Get Network ID
