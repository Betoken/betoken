...
var Web3, avgROI, betoken, betoken_addr, chart, clock, countdownDay, countdownHour, countdownMin, countdownSec, cycleNumber, cyclePhase, displayedKairoBalance, displayedKairoUnit, etherDelta_addr, kairoBalance, kairoTotalSupply, kairo_addr, loadFundData, memberList, networkName, prevCommission, prevROI, proposalList, showCountdown, showTransaction, startTimeOfCycle, supportedProposalList, timeOfChangeMaking, timeOfCycle, timeOfProposalMaking, totalCommission, totalFunds, transactionHash, userAddress, userBalance, web3;
...
loadFundData = function() {
  ...
  web3.eth.getAccounts().then(function(accounts) {
    ...
    return betoken.getKairoBalance(_userAddress).then(function(_kairoBalance) {
  ...
  betoken.getPrimitiveVar("cycleNumber").then(function(_result) {
    ...
    betoken.contracts.groupFund.events.ROI({
      ...
      //Update average ROI
      receivedROICount += 1;
      return avgROI.set(avgROI.get().add(ROI.minus(avgROI.get()).div(receivedROICount)));
    });
    //Example data
    /*chart.data.datasets[0].data = [
      x: "1"
    ...
    chart.update()*/
    return betoken.contracts.groupFund.events.CommissionPaid({
      fromBlock: 0
    }).on("data", function(_event) {
      var commission;
      ...
      return totalCommission.set(totalCommission.get().add(commission));
    });
  });
...
Template.stats_tab.helpers({
  ...
  prev_commission: function() {
    return prevCommission.get().toFormat(2);
  },
  historical_commission: function() {
    return totalCommission.get().toFormat(2);
  }
});
