...
var INPUT_ERR, SEND_TX_ERR, STAKE_BOTH_SIDES_ERR, Web3, allowEmergencyWithdraw, avgROI, betoken, betoken_addr, chart, checkKairoAmountError, clock, commissionRate, copyTextToClipboard, countdownDay, countdownHour, countdownMin, countdownSec, cycleNumber, cyclePhase, displayedKairoBalance, displayedKairoUnit, errorMessage, hasWeb3, kairoAddr, kairoBalance, kairoTotalSupply, kyberAddr, loadFundData, networkName, networkPrefix, paused, prevCommission, prevROI, proposalList, sharesAddr, sharesBalance, sharesTotalSupply, showCountdown, showError, showSuccess, showTransaction, startTimeOfCyclePhase, successMessage, timeBetweenCycles, timeOfChangeMaking, timeOfFinalizing, timeOfProposalMaking, timeOfWaiting, totalCommission, totalFunds, transactionHash, transactionHistory, userAddress, userBalance, web3;
...
loadFundData = function() {
  ...
  betoken.getPrimitiveVar("timeOfFinalizing").then(function(_time) {
    return timeOfFinalizing.set(+_time);
  });
  betoken.getPrimitiveVar("timeBetweenCycles").then(function(_time) {
    return timeBetweenCycles.set(+_time);
  });
  ...
  Promise.all([
    ...
    return betoken.getPrimitiveVar("proposals").then(function(_proposals) {
      var handleAllProposals, handleProposal, i;
      proposals = _proposals;
      if (proposals.length === 0) {
        return;
      }
      handleProposal = function(id) {
        return betoken.getTokenSymbol(proposals[id]).then(function(_symbol) {
          proposals[id].id = id;
          proposals[id].tokenSymbol = _symbol;
          return proposals[id].investment = BigNumber(proposals[id].stake).div(kairoTotalSupply.get()).mul(totalFunds.get());
        });
      };
      handleAllProposals = (function() {
        var j, ref, results;
        results = [];
        for (i = j = 0, ref = proposals.length; (0 <= ref ? j <= ref : j >= ref); i = 0 <= ref ? ++j : --j) {
          results.push(handleProposal(i));
        }
        return results;
      })();
      return Promise.all(getAllSymbols);
    }).then(function() {
      return proposalList.set(proposals);
    });
  });
};
...
Template.transact_box.helpers({
  is_disabled: function(_type) {
    if (cyclePhase.get() !== 0 || (cycleNumber.get() === 1 && _type === "withdraw") || (cycleNumber.get() === 4 && _type === "kairo")) {
      return "disabled";
    }
  },
