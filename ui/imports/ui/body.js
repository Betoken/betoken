...
postLoadAllData = async function() {
  ...
  if (!hasWeb3) {
    ...
    return showError(METAMASK_LOCKED_ERR);
  }
};

loadRanking = async function() {
  var _event, events, j, k, len, len1, rank, ranking, results, user, userKairoBalance;
  // load Kairo ranking
  kairoRanking.set([]);
  ...
  for (j = 0, len = events.length; j < len; j++) {
    ...
    rank = 1;
    for (k = 0, len1 = ranking.length; k < len1; k++) {
      user = ranking[k];
      user.rank = rank;
      rank += 1;
    }
    results.push(kairoRanking.set(ranking));
  }
  return results;
};
...
Template.top_bar.helpers({
  ...
  need_web3: function() {
    if (userAddress.get() !== "0x0" && hasWeb3) {
      return "";
    } else {
      return "disabled";
    }
