...
loadFundData = function() {
  var investments, receivedROICount;
  investments = [];
  receivedROICount = 0;
  //Get Network ID
  ...
  web3.eth.getAccounts().then(function(accounts) {
    ...
    Promise.all([
      ...
      return betoken.getInvestments(userAddress.get()).then(function(_proposals) {
        ...
        handleProposal = function(id) {
          return betoken.getTokenSymbol(investments[id]).then(function(_symbol) {
            investments[id].id = id;
            investments[id].tokenSymbol = _symbol;
            return investments[id].investment = BigNumber(investments[id].stake).div(kairoTotalSupply.get()).mul(totalFunds.get());
          });
        };
        handleAllProposals = (function() {
          var j, ref, results;
          results = [];
          for (i = j = 0, ref = investments.length; (0 <= ref ? j <= ref : j >= ref); i = 0 <= ref ? ++j : --j) {
            results.push(handleProposal(i));
          }
          return results;
        })();
        return Promise.all(handleAllProposals);
      }).then(function() {
        return investmentList.set(investments);
      });
    });
    ...
    getTransactionHistory = function(_type) {
      return betoken.contracts.betokenFund.getPastEvents(_type, {
        ...
        var _event, data, entry, j, len, results;
        results = [];
