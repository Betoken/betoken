...
(function() {
  var BetokenFund, BigNumber, CO, CPD, CompoundOrder, DAI, DAI_PRICE, DAY, ETH_PRICE, EXIT_FEE, FUND, KN, KRO, LONG_LEVERAGE, MiniMeToken, MiniMeTokenFactory, OMG_PRICE, PHASE_LENGTHS, PRECISION, SHORT_LEVERAGE, ST, TK, TestCompound, TestKyberNetwork, TestToken, TestTokenFactory, ZERO_ADDR, bnToString, calcRegisterPayAmount, epsilon, epsilon_equal, timeTravel;
  ...
  contract("first_cycle", function(accounts) {
    ...
    it("sell_compound_orders", async function() {
      var MAX_PRICE, account2, compound, dai, fundDAIBlnce, kro, kroBlnce, longOrder, prevFundDAIBlnce, prevKROBlnce, shortOrder, stake, token;
      kro = (await KRO(this.fund));
      token = (await TK("OMG"));
      dai = (await DAI(this.fund));
      MAX_PRICE = bnToString(OMG_PRICE * 2);
    ...
    return it("next_phase", async function() {
      await timeTravel(PHASE_LENGTHS[0]);
  ...
  contract("param_setters", function(accounts) {
    ...
    it("decrease_only_proportion_setters", async function() {
      ...
      // valid
      await this.fund.changeExitFeeRate(exitFeeRate.idiv(2), {
        from: owner
      });
