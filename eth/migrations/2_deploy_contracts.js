    ...
    return deployer.then(async function() {
      ...
      switch (network) {
        ...
        case "mainnet":
          ...
          await deployer.deploy(BetokenLogic);
          // deploy BetokenFund contract
          await deployer.deploy(BetokenFund, config.KAIRO_ADDR, ShareToken.address, config.DEVELOPER_ACCOUNT, config.phaseLengths, bnToString(config.devFundingRate), ZERO_ADDR, config.DAI_ADDR, config.KYBER_ADDR, CompoundOrderFactory.address, BetokenLogic.address);
          betokenFund = (await BetokenFund.deployed());
          ...
          return (await fund.transferOwnership(config.DEVELOPER_ACCOUNT));
      }
    });
  };
